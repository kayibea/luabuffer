...
int l_buffer_from(lua_State *L) {
  ...
  case LUA_TUSERDATA: {
    if (luaL_testudata(L, 1, BUFFER_MT))
      return buffer_alloc_fbuffer(L);
    [[fallthrough]];
  }
  default: {
...
static int buffer_alloc_ftable(lua_State *L) {
  ...
  buf_t *buf = lua_newuserdata(L, sizeof(buf_t));
  buf->size = len;
  buf->buffer = malloc(len);
  if (!buf->buffer)
    return throw_luaoom(L, len);
...
static int buffer_alloc_fbuffer(lua_State *L) {
  buf_t *src = luaL_checkudata(L, 1, BUFFER_MT);
  buf_t *buf = lua_newuserdata(L, sizeof(buf_t));

  buf->size = src->size;
  buf->buffer = malloc(src->size);
  if (!buf->buffer)
    return throw_luaoom(L, src->size);
...
static int buffer_alloc_fstring(lua_State *L) {
  ...
  if (strcasecmp(encoding, ENCODING_UTF8) == 0) {
    ...
    if (!decoded)
      return throw_luaoom(L, in_len);
    memcpy(decoded, input, in_len);
    out_len = in_len;
  } else {
    return luaL_error(L, "Bad encoding (%s)\n", encoding);
  }
...
int l_buffer_alloc(lua_State *L) {
  ...
  if (!buf->buffer)
    return throw_luaoom(L, buf->size);
  ...
  if (canfill) {
    ...
    switch (type) {
    case LUA_TUSERDATA: {
      buf_t *src = luaL_checkudata(L, 2, BUFFER_MT);
    ...
    case LUA_TSTRING: {
      ...
      if (strcasecmp(encoding, ENCODING_UTF8) == 0) {
        ...
        return luaL_error(L, "Bad encoding (%s)", encoding);
      }

      for (size_t i = 0; i < buf->size; i++)
        buf->buffer[i] = data[i % data_len];

      if (data != (uint8_t *)fill_str)
        if (data)
          free(data);
    ...
    default:
      return luaL_error(L,
                        "Invalid fill type: must be number, string, or buffer");
    }
  }
